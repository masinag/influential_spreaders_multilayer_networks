    ...
    for filename in os.listdir(path):
        if filename.endswith('.sim'):
            ...
            with open(path + '/' + filename) as f:
                num_nodes, sum_nodes = map(int, f.readline().split())
...
def evaluate(sim_score, alg_scores):
    ret = {}
    for alg_name, alg_score in alg_scores.items():
        k, _ = kendalltau(alg_score, sim_score)
        if np.isnan(k):
            print(alg_score, sim_score)
            sys.exit(0)
        else: 
            ret[alg_name] = float(k)
    return ret
    # return {alg_name : float(kendalltau(alg_score, sim_score, nan_policy='raise')[0]) for alg_name, alg_score in alg_scores.items()}
...
if __name__ == "__main__":
    ...
    for dir in DATASETS_DIRS:
        for net in os.listdir(dir):
            ...
            #         kt[sp] = kendalltau(simulations_ranking[sp], algorithm_ranking)
